...
def filter_out_distant_point(poly, xs, ys, threshold):

    xx, yy = [], []
    for x, y in zip(xs, ys):
        v = Polynomial.evaluate(poly, x)
        if abs(y-v) <= threshold:
            xx.append(x)
            yy.append(y)
...
def find_curv(xs, ys, rng):
    ...
    while True:
        poly = Polynomial.fit(xs, ys, order)
        print "fit poly:", poly

        maxdiff, mindiff = eval_maxmindiff(poly, xs, ys)
        curr_rng = maxdiff - mindiff
        print "diffs:", maxdiff, mindiff, "range size:", curr_rng

        if maxdiff - mindiff <= rng:
            poly[0] += mindiff
            break

        v = Polynomial.variance(poly, xs, ys)

        v = v**.5
        thre = v * 1.2
        print "threshold:", thre

        xs2, ys2 = filter_out_distant_point(poly, xs, ys, thre)
        print "NO. points for next round:", len(xs), len(xs2)
        xs, ys = xs2, ys2

    return poly, xs, ys, resample_points(poly, xs, ys)

def find_curv_bsearch(xs, ys, rng):
...
def find_polys(xs, ys):
    ...
    while len(xs) > 0:
        ...
        print "start==="
        print
        poly, x2, y2, newys = find_curv(xs, ys, want_range_size)
        print "curve:", poly, len(x2)
        print "points:", x2
        ...
        xs, ys = a, b
        print "points left:", xs

        polys.append(poly)
        points.append([x2, newys])
    return polys, points
...
for l in Polynomial.plot(polys, rng, height=40, yrange=[0, 1500], points=zip(xs, ys)):
    print l
